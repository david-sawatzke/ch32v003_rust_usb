...
static const uint8_t mouse_hid_desc[] = {
    // From http://eleccelerator.com/tutorial-about-usb-hid-report-descriptors/
...
static const uint8_t keyboard_hid_desc[] = {
    ...
    HID_USAGE_PAGE(
        HID_USAGE_PAGE_KEYBOARD), //     USAGE_PAGE (Keyboard)(Key Codes)
    ...
    HID_REPORT_SIZE(8),   //     REPORT_SIZE (8)
    HID_LOGICAL_MIN(0),   //     LOGICAL_MINIMUM (0)
    HID_LOGICAL_MAX(167), //     LOGICAL_MAXIMUM (167)  (Normally would be 101,
                          //     but we want volume buttons)
    HID_USAGE_PAGE(
        HID_USAGE_PAGE_KEYBOARD), //     USAGE_PAGE (Keyboard)(Key Codes)
    HID_USAGE_MIN(0x00),          //     USAGE_MINIMUM (0)
    HID_USAGE_MAX(
        167),     //     USAGE_MAXIMUM (Keyboard Application)(101) (Now 167)
    HID_INPUT(0), //   INPUT (Data,Ary,Abs)
    HID_COLLECTION_END, // END_COLLECTION
};

// Ever wonder how you have more than 6 keys down at the same time on a USB
// keyboard?  It's easy. Enumerate two keyboards!
...
const static struct usb_string_descriptor_struct string1
    __attribute__((section(".rodata"))) = {sizeof(STR_MANUFACTURER), 3,
                                           STR_MANUFACTURER};
const static struct usb_string_descriptor_struct string2
    __attribute__((section(".rodata"))) = {sizeof(STR_PRODUCT), 3, STR_PRODUCT};
const static struct usb_string_descriptor_struct string3
    __attribute__((section(".rodata"))) = {sizeof(STR_SERIAL), 3, STR_SERIAL};
